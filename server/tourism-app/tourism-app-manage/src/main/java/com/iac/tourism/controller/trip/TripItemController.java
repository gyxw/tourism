...
import com.iac.tourism.service.trip.TripItemService;


@Controller
@RequestMapping("trip/tripitem")
public class TripItemController extends BasicController {
	...
	public String list(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<TripItem> page = tripItemService.findPage(searchParams, pageNumber, pageSize, sortType);
				
		model.addAttribute("page", page);
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "trip/tripitemList";
	}
	
	/** 进入新增 */
	@RequestMapping(value="/create", method = RequestMethod.GET)
	public String createForm(ModelMap model) {
		return "trip/tripitemForm";
	}
	
	/** 编辑 */
	@RequestMapping(value="update/{id}", method = RequestMethod.GET)
	public String updateForm(ModelMap model, @PathVariable java.lang.Long id) {
		TripItem tripItem = tripItemService.get(id);
		model.addAttribute("tripItem",tripItem);
		return "trip/tripitemForm";
	}
	
	/** 保存更新,@Valid标注spirng在绑定对象时自动为我们验证对象属性并存放errors在BindingResult  */
	@RequestMapping(value = "update", method = RequestMethod.POST)
	public String update(@Valid @ModelAttribute("tripItem") TripItem tripItem, 
			RedirectAttributes redirectAttributes, BindingResult errors, ModelMap model) {
		tripItemService.save(tripItem);
		redirectAttributes.addFlashAttribute("message", "更新成功");
		return "redirect:/trip/tripitem";
	}
	...
	public String batchDelete(ModelMap model,@RequestParam("items") java.lang.Long[] items,
		...
		for(int i = 0; i < items.length; i++) {
			tripItemService.delete(items[i]);
		}
		redirectAttributes.addFlashAttribute("message", "删除成功");
		return "redirect:/trip/tripitem";
	}
	 ...
	 */
	@ModelAttribute
	public void getTripItem(@RequestParam(value = "id", defaultValue = "-1") java.lang.Long id, Model model) {
		if (id != -1) {
			model.addAttribute("tripItem", tripItemService.get(id));
		}
	}
}
