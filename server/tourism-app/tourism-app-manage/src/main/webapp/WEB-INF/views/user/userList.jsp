...
<div class="row-fluid sortable ui-sortable">	
	<div class="box span12">
		<div class="box-header well" data-original-title="">
			<form action="${ctx}/user/user" method="post">
			<h2>
				<span class="ajax-link btn btn-primary" action="${ctx}/user/user/create"><i class="icon-plus"></i> 用户</span>
		...
		<table class="table table-bordered table-striped table-condensed">
			<thead>
				<tr role="row">
					...
					<th sortColumn="login_name" >loginName</th>
					<th sortColumn="password" >password</th>
					<th sortColumn="salt" >salt</th>
					<th sortColumn="mobile" >mobile</th>
					<th sortColumn="sex" >sex</th>
					<th sortColumn="nick_name" >nickName</th>
					<th>操作</th>
				</tr>
			</thead>   
			<tbody>
					...
					<td class="center ">
						<shiro:hasPermission name="user:edit">
							<a class="btn btn-info ajax-link" style="padding: 0;"
								action="${ctx}/user/user/update/${item.id}" title="编辑">
								<i class="icon-edit icon-white"></i> </a>
							<a class="btn btn-danger ajax-link" style="padding: 0;"
								action="${ctx}/user/user/delete/${item.id}" title="删除">
	   ...
	   <tags:pagination page="${page}" paginationSize="5" action="${ctx}/user/user" />
	</div>
	</div><!--/span-->
</div>
